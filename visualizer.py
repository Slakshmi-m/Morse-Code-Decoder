...
===================================================
Five panels on ONE screen, exactly as required by the assignment:
  ...
  Panel 4 (lower-left) : Binary Signal — decoded 0/1 envelope (ON/OFF)
  Panel 5 (lower-right): Dit / Dah Bar Chart — duration histogram coloured
                         green for dits, red for dahs, blue for spaces

What each panel shows the examiner
------------------------------------
- Oscilloscope: the raw noisy audio — dots and dashes are visible as
  groups of fast oscillations separated by silence.
- FFT Spectrum: proves the bandpass filter is centred on the right tone
  (e.g. 700 Hz). Yellow dashed line marks the auto-detected carrier.
- Waterfall: the "SDR-style" spectrogram. The Morse code appears as a
  bright horizontal stripe. Dot and dash patterns are clearly visible
  as short and long bright segments along the carrier frequency.
- Binary Signal: the envelope after smoothing and threshold detection.
  Clean rectangular pulses — each ON block is one dot or dash.
- Dit/Dah histogram: proves the decoder correctly separates dots (short
  pulses, green), dashes (long pulses, red), and spaces (blue) using
  the adaptive unit-time algorithm in engine.py.

Usage
-----
    python visualizer.py                          # uses noisy_paragraph.wav
    python visualizer.py my_audio.wav             # any WAV file
    python visualizer.py my_audio.wav out.png     # save to PNG instead
"""
...
PANEL_BG  = "#11111F"   # axes background
GREEN     = "#00FF88"   # waveform / dit bars
ORANGE    = "#FF8800"   # FFT fill
YELLOW    = "#FFE500"   # carrier marker
CYAN      = "#00CFFF"   # binary signal
RED       = "#FF4466"   # dah bars
BLUE      = "#4488FF"   # space bars
GREY      = "#555577"   # grid / spines
LABEL_COL = "#AAAACC"   # axis labels / tick text
WHITE     = "#E8E8FF"   # titles
...
class Visualizer:
    ...
    Parameters
    ----------
    sample_rate : int   Audio sample rate in Hz (typically 8000).
    data        : ndarray   Raw PCM samples (int16 or float32, mono or stereo).
    title       : str   Label shown in the window title and suptitle.
    max_seconds : float Clip audio to this many seconds for display (default 10).
    freq_min    : int   Lower bound of frequency display range in Hz.
    freq_max    : int   Upper bound of frequency display range in Hz.
    """
    ...
    def __init__(
        ...
        # ---- pre-compute all derived signals once ----
    ...
    def show(self, save_path: str = None) -> None:
        """Render the full dashboard.  Pass save_path to write a PNG."""
    ...
    def get_peak_frequency(self) -> float:
        """Return the dominant Morse carrier frequency in Hz."""
    ...
    def _bandpass(self, data: np.ndarray) -> np.ndarray:
        """Narrow bandpass centred on the detected carrier tone."""
    ...
    def _smooth_envelope(self, data: np.ndarray) -> np.ndarray:
        """Rectify + box-smooth to get amplitude envelope."""
        win = max(1, int(self.sr * 0.006))          # 6 ms window
        return np.convolve(np.abs(data),
                           np.ones(win) / win, mode="same")

    def _threshold(self, env: np.ndarray) -> np.ndarray:
        """Hard threshold → binary on/off signal."""
        lo = np.percentile(env, 5)
        hi = np.percentile(env, 95)
        thresh = lo + (hi - lo) * 0.45
        return (env > thresh).astype(np.int8)

    def _get_segments(self, binary: np.ndarray):
        """Return list of (state, duration_samples) run-length segments."""
        changes = np.diff(binary.astype(int))
        idx = list(np.where(changes != 0)[0] + 1)
        idx = [0] + idx + [len(binary)]
        return [(int(binary[idx[i]]), idx[i+1] - idx[i])
                for i in range(len(idx) - 1)]

    def _estimate_unit(self, segments) -> float:
        """Estimate dit duration in samples using the lower-half mean."""
    ...
    def _fft_spectrum(self, data: np.ndarray):
        """One-sided Hann-windowed magnitude spectrum."""
        win = np.hanning(len(data))
        mags = np.abs(np.fft.rfft(data * win))
        freqs = np.fft.rfftfreq(len(data), d=1.0 / self.sr)
        return freqs, mags
    ...
    def _build_figure(self):
        ...
        fig.suptitle(
            f"Morse Signal Dashboard  —  {self.title}",
            color=WHITE, fontsize=13, fontweight="bold", y=0.985,
        )

        # ---- layout: 3 rows, 2 columns ----
        # Row 0: oscilloscope (left) | FFT spectrum (right)
        # Row 1: waterfall / spectrogram (spans full width)
        # Row 2: binary signal (left) | dit-dah histogram (right)
    ...
    # ================================================================
    # Panel 1 — Oscilloscope (time-domain waveform)
    # ================================================================

    def _panel_waveform(self, ax) -> None:
    ...
    def _panel_fft(self, ax) -> None:
        ...
        if len(m) > 0:
            pk_f = f[np.argmax(m)]
            pk_m = m[np.argmax(m)]
            ax.axvline(pk_f, color=YELLOW, linewidth=1.3, linestyle="--", alpha=0.9)
            ax.text(pk_f + (self.freq_max - self.freq_min) * 0.02,
                    pk_m * 0.85,
                    f"Carrier\n{pk_f:.0f} Hz",
    ...
    # ================================================================
    # Panel 3 — Spectrogram / Waterfall (full width)
    # ================================================================

    def _panel_waterfall(self, ax) -> None:
        ...
        ax.set_title(
            "③ Spectrogram / Waterfall  —  Morse pattern visible as bright stripe",
    ...
    # ================================================================
    # Panel 4 — Binary (On/Off) Signal
    # ================================================================

    def _panel_binary(self, ax) -> None:
        t = np.linspace(0, len(self.display) / self.sr, len(self._binary))

        # Draw as filled step plot so ON blocks are solid rectangles
        ...
        ax.set_title("④ Binary Signal  (Envelope → ON/OFF)", color=WHITE,
                     fontsize=9, pad=4, loc="left")
        ax.set_xlabel("Time (s)", color=LABEL_COL, fontsize=8)
    ...
    # ================================================================
    # Panel 5 — Dit / Dah Duration Histogram
    # ================================================================

    def _panel_ditdah_hist(self, ax) -> None:
        unit = self._unit
        if unit <= 0:
            unit = 1.0

        # Convert sample durations to milliseconds
        ms_per_sample = 1000.0 / self.sr

        dits   = [d * ms_per_sample for s, d in self._segments
                  if s == 1 and d < unit * 2]
        dahs   = [d * ms_per_sample for s, d in self._segments
                  if s == 1 and d >= unit * 2]
        spaces = [d * ms_per_sample for s, d in self._segments
                  if s == 0 and d >= unit * 0.5]
        ...
        max_ms = max(all_vals)
        bins   = np.linspace(0, min(max_ms, unit * 12 * ms_per_sample), 36)
        ...
        # Mark the unit boundary (dit/dah split)
        split_ms = unit * 2 * ms_per_sample
        ax.axvline(split_ms, color=YELLOW, linewidth=1.2,
                   linestyle="--", alpha=0.9)
        ...
        legend = ax.legend(fontsize=7.5, facecolor="#1A1A2E",
...
def visualize_wav(file_path: str, save_path: str = None) -> None:
    """Load a WAV and display (or save) the full 5-panel dashboard."""
    try:
        fs, data = wavfile.read(file_path)
    except Exception as e:
        print(f"[Visualizer] Cannot read '{file_path}': {e}")
        return

    viz = Visualizer(fs, data, title=file_path)
    print(f"[Visualizer] Carrier frequency: {viz.get_peak_frequency():.1f} Hz")
    print(f"[Visualizer] Estimated dit length: {viz._unit / fs * 1000:.1f} ms")
    viz.show(save_path=save_path)


if __name__ == "__main__":
    wav   = sys.argv[1] if len(sys.argv) > 1 else "noisy_paragraph.wav"
    out   = sys.argv[2] if len(sys.argv) > 2 else None
    visualize_wav(wav, out)
